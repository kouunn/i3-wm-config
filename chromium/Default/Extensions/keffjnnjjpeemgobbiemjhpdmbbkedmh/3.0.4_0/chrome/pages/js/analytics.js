...
var chrome_v=versionPraser();

(function() {
    "use strict";
    if (typeof localStorage.uuid === 'undefined') {
        // http://stackoverflow.com/questions/105034/how-to-create-a-guid-uuid-in-javascript
        unblock_boundary.uuid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
            var r = Math.random()*16|0;
            var v = c === 'x' ? r : (r&0x3|0x8);
            return v.toString(16);
        });
        localStorage.uuid = unblock_boundary.uuid;
    } else {
        unblock_boundary.uuid = localStorage.uuid;
    }
}());
...
function ga_collect_data(type, data) {
    ...
    var payload = 'v=1&' 
                + 'tid=UA-72599399-5&' //unblock plugin
                + 'cid='+ unblock_boundary.uuid + '&' 
                + 'aip=1&'  // anonymize IP
                + 't=' + type + '&'
                + data;
    xhr.send(payload);
...
function ga_report_important(ratio_name, ratio_value, collection_rate) {
    ga_report_event(ratio_name, ratio_value, collection_rate);
    // ga_report_event(ratio_name, ratio_value, 1.0);
}

function ga_report_100(ratio_name, ratio_value) {
    ga_report_event(ratio_name, ratio_value, 1.0);
}
...
function ga_report_error(error_name, error_desc) {
    "use strict";
    var ext_v=unblock_boundary.lastest_new_version || 'unknown';
    error_desc=error_desc + " chrome_v="+chrome_v+" ext_v="+ext_v;
